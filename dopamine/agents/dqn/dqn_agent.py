...
NATURE_DQN_OBSERVATION_SHAPE = (3,1)  # Size of downscaled Atari 2600 frame.
...
class DQNAgent(object):
  ...
  def _build_networks(self):
    ...
    self.batch_state_action = tf.concat([self._replay.states,self._replay.actions[:,None,None,None]],1)#32*4*1*1
    self.batch_online_q_value = self.online_Critic(self.batch_state_action).q_value#32*1
    self.batch_next_action = self.target_Actor(self._replay.next_states).action#32*1
    self.batch_next_state_action = tf.concat([self._replay.next_states,self.batch_next_action[:,None,None]],1)#32*4*1*1
    self.batch_next_q_value = self.target_Critic(self.batch_next_state_action).q_value#32*1
    self.batch_y_q_value = self.cumulative_gamma * self.batch_next_q_value * (
        1. - tf.cast(self._replay.terminals[:,None], tf.float32)) + self._replay.rewards[:,None]
    ...
    self.batch_online_action = self.online_Actor(self._replay.states).action
    self.batch_online_state_action = tf.concat([self._replay.states,self.batch_online_action[:,None,None]],1)
    self.batch_online_q_value2 = self.online_Critic(self.batch_online_state_action).q_value
    gradients = tf.gradients(self.batch_online_q_value2,self.batch_online_state_action)#32*4*1*1
    gradients = tf.squeeze(gradients)
    self.batch_size = tf.shape(self._replay.rewards)[0]
    first_indices = tf.range(self.batch_size)[:,None]
    second_indices = tf.zeros([self.batch_size,1], dtype=tf.int32) -1
    self.indices = tf.concat([first_indices,second_indices],1)
    self.action_gradients = tf.gather_nd(gradients,self.indices)#(32,)
    #here we build optimizer
    #first build trainable parameter set
    self.build_trainable_param()
    #second build critic optimizator
    self.online_critic_update = online_critic_optimizer.minimize(tf.reduce_mean(online_critic_loss))
    #At last, build actor optimizator
    self.online_actor_gradients = tf.gradients(self.batch_online_action,self.online_actor_param,-gradients)
    ...
    if self.summary_writer is not None:
      with tf.variable_scope('Losses'):
        tf.summary.scalar('online_critic_loss', tf.reduce_mean(online_critic_loss))
  ...
  def _build_sync_op(self):
    ...
    for (origin,target) in zip(self.target_actor_param,self.actor_target_param):
      sync_actor_ops.append(origin.assign(target,use_locking=True))
    for (origin,target) in zip(self.target_critic_param,self.critic_target_param):
      sync_critic_ops.append(origin.assign(target,use_locking=True))
  ...
  def _select_action(self):
    ...
    Returns:
       int, the selected action.
    """
    a = self._sess.run([self.online_Action], {self.state_ph : self.state})[0][0]
    if self.eval_mode:
